...
const MealItem: React.FC<{ meal: Meal }> = React.memo(({ meal }) => {
  ...
  const { setMealToEdit, setActiveModal } = useUI();
  const { name, nutrition, imageUrl, time, source } = meal;
  ...
  return (
    <Card className="p-0 flex items-center overflow-hidden group/meal">
      <div className="flex-1 p-4 cursor-pointer" onClick={handleEdit}>
        <div className="flex gap-4 items-center">
            {imageUrl && <img src={imageUrl} alt={name} className="w-16 h-16 rounded-lg object-cover flex-shrink-0" />}
