...
const AllNutrientsModal: React.FC<{ isOpen: boolean; onClose: () => void; goals: NutritionInfo; onSave: (newGoals: NutritionInfo) => void; }> = ({ isOpen, onClose, goals, onSave }) => {
    ...
    const allNutrients = useMemo(() => ['calories', 'protein', 'carbs', 'fat', 'waterGoal', ...Object.keys(nutrientMetadata)], []);
            ...
            <div className="flex-1 overflow-y-auto space-y-4 p-1 pr-2 mt-4">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-3">
                    {allNutrients.map(key => {
                        const meta = (nutrientMetadata as any)[key] || { label: key.charAt(0).toUpperCase() + key.slice(1), unit: key === 'calories' ? 'kcal' : (key === 'waterGoal' ? 'ml' : 'g')};
...
const Profile: React.FC = () => {
  const { dailyGoal, setDailyGoal, userProfile, setUserProfile, addWeightEntry } = useData();
